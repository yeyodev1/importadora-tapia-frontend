      ...
      <div v-if="open && pedido" class="backdrop" @click.self="emit('close')">
        <div class="wrap">
          <div id="comprobante" class="doc">
            ...
            <div v-if="pedido.fotos?.length || pedido.fotoUrl" class="doc__fotos">
              <a
                v-for="(url, k) in pedido.fotos?.length ? pedido.fotos : [pedido.fotoUrl as string]"
                :key="url"
                :href="url"
                target="_blank"
                rel="noopener"
                class="doc__foto"
              >
                <img :src="url" :alt="`Foto ${k + 1} de la orden de pedido`" />
              </a>
            </div>
...
.doc {
  ...
  &__fotos { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 14px; }
  &__foto { display: block; img { max-height: 140px; border-radius: 8px; border: 1px solid #E5E9F0; } }
